...
public record GenerateGeneric(double baseChance, Either<Block, TagKey<Block>>[] set, ResourceKey<ConfiguredFeature<?, ?>>[] featureSet, Block placeBlock,
    ...
    public static final MapCodec<GenerateGeneric> CODEC = RecordCodecBuilder.mapCodec(instance ->
            instance.group(Codec.DOUBLE.fieldOf("base_chance").forGetter(GenerateGeneric::baseChance),
                            ...
                            PBNBTHelper.arrayCodec(ResourceKey.codec(Registries.CONFIGURED_FEATURE), () -> (ResourceKey<ConfiguredFeature<?,?>>[]) new ResourceKey[0]).fieldOf("feature_set").forGetter(GenerateGeneric::featureSet),
    ...
    public GenerateGeneric(double baseChance, Either<Block, TagKey<Block>>[] exclusions, ResourceKey<ConfiguredFeature<?, ?>> feature, Block placeBlock) {
        this(baseChance, exclusions, new ResourceKey[] {feature}, placeBlock, false);
    }
    @Override
