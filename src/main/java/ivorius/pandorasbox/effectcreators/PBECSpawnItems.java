...
public record PBECSpawnItems(IValue number, IValue ticksPerItem, EitherArrayList<RandomizedItemStack, RandomizedItemTag> items, ZValue spawnsFromEffectCenter, Optional<ValueThrow> valueThrow, Optional<ValueSpawn> valueSpawn) implements PBEffectCreator {
    ...
    public static ItemStack[] getItemStacks(RandomSource random, List<RandomizedItemStack> items, int number, boolean split, boolean mixUp, int enchantLevel, boolean giveNames) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            enchantItemStack(enchantLevel, random, stack);
    ...
    public static void enchantItemStack(int enchantLevel, RandomSource random, ItemStack stack) {
        if (enchantLevel > 0) {
            List<EnchantmentInstance> enchantments = EnchantmentHelper.selectEnchantment(random, stack, enchantLevel, true);

            if (enchantments.isEmpty()) {
                enchantments = EnchantmentHelper.selectEnchantment(random, new ItemStack(Items.BOOK), enchantLevel, true);
            }
    ...
    public PBEffect constructEffect(Level world, double x, double y, double z, RandomSource random) {
        ...
        ItemStack[] stacks = getItemStacks(random, PandorasBoxHelper.assembleRandomisedStacks(BuiltInRegistries.ITEM, BuiltInRegistries.BLOCK, items), number, random.nextInt(3) != 0, true, 0, false);
