 ...
 */
public record PBECConvertToHomo(DValue range) implements PBEffectCreator {
    public static final List<BlockMapper> HOMO_MAPPERS;
    public static final List<EntitySpawner> HOMO_SPAWNERS;
    public static final List<FeatureGenerator> HOMO_FEATURES;
    static {
        HOMO_MAPPERS = new ArrayList<>();
        HOMO_MAPPERS.add(new SimpleConvertMapper(new Either[] {Either.right(BlockTags.SNOW), Either.right(BlockTags.FIRE),
        ...
        HOMO_MAPPERS.add(new SurfaceMapper(new Either[] {Either.right(PandorasBox.ALL_TERRACOTTA), Either.right(ConventionalBlockTags.STONES),
        ...
        HOMO_MAPPERS.add(new SimpleConvertMapper(new Either[] {Either.right(ConventionalBlockTags.OBSIDIANS), Either.left(Blocks.LAVA), Either.left(Blocks.ICE)}, Blocks.WATER));
        HOMO_SPAWNERS = new ArrayList<>();
        HOMO_SPAWNERS.add(new SpawnRandom("pbspecial_colorful_sheep", 1.0f / (20 * 20)));
        HOMO_FEATURES = new ArrayList<>();
        HOMO_FEATURES.add(new GenerateHomo());
    }
    public static final MapCodec<PBECConvertToHomo> CODEC = DValue.CODEC.fieldOf("range").xmap(PBECConvertToHomo::new, PBECConvertToHomo::range);
    ...
    public PBEffect constructEffect(Level world, double x, double y, double z, RandomSource random) {
        ...
        return new PBEffectGenerate(time, range, 3, PandorasBoxHelper.getRandomUnifiedSeed(random), new SimpleConvertEffect(Optional.of(Biomes.FLOWER_FOREST), HOMO_MAPPERS, HOMO_FEATURES, HOMO_SPAWNERS));
    }
