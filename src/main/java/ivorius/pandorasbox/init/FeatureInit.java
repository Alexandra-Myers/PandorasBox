...
public class FeatureInit {
    ...
    private static <C extends FeatureConfiguration> Feature<C> register(String name, Feature<C> feature) {
        return Registry.register(BuiltInRegistries.FEATURE, ResourceKey.create(BuiltInRegistries.FEATURE.key(), Identifier.fromNamespaceAndPath(PandorasBox.MOD_ID, name)), feature);
    }
    private static ResourceKey<@NotNull ConfiguredFeature<?, ?>> createKey(String name) {
        return ResourceKey.create(Registries.CONFIGURED_FEATURE, Identifier.fromNamespaceAndPath(PandorasBox.MOD_ID, name));
    }
