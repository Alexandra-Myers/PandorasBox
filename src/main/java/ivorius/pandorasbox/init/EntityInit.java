...
public class EntityInit {
    public static final EntityType<FunctionalGiant> GIANT = register("giant", EntityType.Builder.of(FunctionalGiant::new, MobCategory.MONSTER).sized(3.6F, 12.0F).eyeHeight(10.44F).ridingOffset(-3.75F).clientTrackingRange(10).notInPeaceful());
    ...
    private static <T extends Entity> EntityType<T> register(String name, EntityType.Builder<T> entityType) {
        ResourceKey<EntityType<?>> resourceKey = ResourceKey.create(BuiltInRegistries.ENTITY_TYPE.key(), Identifier.fromNamespaceAndPath(PandorasBox.MOD_ID, name));
